...
class ReservationsStack(Stack):
    def __init__(self, scope: Construct, cid: str, *, config: dict, **kw):
        ...
        timeout = lambda_config.get("timeout", 120)
        ...
        fn = _lambda.Function(
            self, "ReservationsSyncFn",
            function_name=f"{client_name}-{env_name}-lambda_reservations_sync_g4h",
            runtime=_lambda.Runtime.PYTHON_3_12,
            handler="handler.handler",
            code=_lambda.Code.from_asset("functions/reservations"),
        ...
        features = config.get("client", {}).get("features", {})
        if "reservations" in features:
            sync_interval = features["reservations"].get("syncIntervalMinutes", 30)

        events.Rule(
            self, "ReservationsSchedule",
            schedule=events.Schedule.rate(Duration.minutes(sync_interval))
    ...
    def _get_lambda_environment_variables(self, config: dict) -> dict:
        ...
        if "reservations" in features:
            ...
            env_vars["RESERVATIONS_SYNC_ENABLED"] = str(reservations.get("syncEnabled", True)).lower()
            env_vars["RESERVATIONS_SYNC_INTERVAL"] = str(reservations.get("syncIntervalMinutes", 30))
